...
Untuk melihat distribusi data, Misalkan dengan histogram, dengan
range=5, terlihat kalau distribusi data cukup normal.
...
Dengan kata lain, membuat model dalam grafik. Metodenya bisa
macam-macam, bisa pakai lm.
...
ggplot(data = evals, aes(x = age, y = score, colour = ethnicity)) +
  geom_jitter() +
  scale_color_manual(values = c("minority"="green", "not minority"= "purple")) +
  geom_smooth(method = "lm") +
  labs(
    x = "usia",
    y = "skor",
