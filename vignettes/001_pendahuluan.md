...
```r
huruf_vokal <- c("a", "i", "u", "e", "o") # objek data tersimpan dengan nama 'huruf_vokal', namun tidak tercetak pada konsol, tanda kurung untuk mencetak
...
```r
dim(iris) # Untuk mengecek dimensi data, mis 150 5, artinya 150 baris 5 kolom
```
...
```r
str(iris) # Untuk menampilkan struktur dari data, melihat hasilnya.
```
...
```r
colnames(iris) # untuk mengecek nama setiap kolom
```
...
```r
head(iris) # mencetak 6 baris pertama dari data dimana defaultnya ada 6
```
...
```r
tail(iris, 10) # mencetak 10 baris terakhir
```
...
```r
summary(iris) # menampilkan semua rekapan atau rangkuman hasil perhitungan setiap kolom
```
...
```r
iris[1, ] # subset data pada baris 1 (baris, kolom)
```
...
```r
iris[c(1, 6, 12),] # mencetak baris 1,6,12, dan semua kolomnya
```
...
```r
iris[ ,2] # subset atau ekstrak data pada kolom 2,mencetak semua baris iris tapi hanya kolom kedua
```
...
```r
iris[2,2] # mencetak iris pada baris 2 kolom 2, untuk menjaga integritas data framenya tambahin lagi down=false
```
...
```r
length(iris$Petal.Width) # mengecek berapa banyak konten vektor kolom petal.width
```
...
```r
levels(iris$Species) # level dari vektor
```
...
```r
length(levels(iris$Species)) #mengecek berapa banyak konten vektor di kolom spesies
```
...
```r
unique(iris$Species) # leve karakter dari vektor
```
...
```r
length(unique(iris$Species)) # mengecek berapa banyak data yang berbeda di kolom spesies
```
...
```r
mean(iris$Sepal.Length) # menghitung rata-rata di kolom sepal.length
```
...
```r
sd(iris$Sepal.Length) # menghitung sd di kolom sepal.length
```
...
```r
median(iris$Sepal.Width) # menghitung median di kolom sepal.width
```
...
```r
sum(iris$Petal.Length) # menghitung jumlah di kolom petal.length
```
...
```r
plot(iris) # untuk ngeplot data, fungsi changing the environment
```

![](001_pendahuluan_files/figure-html/unnamed-chunk-4-1.png)<!-- -->

```r
cor(iris[, -5]) # menghitung koefisien korelasi dari dataset iris
```
...
```r
write.csv(iris, file = "iris.csv", row.names = FALSE) # menyimpan data sebagai csv, fungsi changing the environment
```

Dari catatan ringkas mengenai fungsi-fungsi di atas, dapatkah Anda menebak mana fungsi yang berjenis '*changing the environment*' dan mana yang berjenis '*calculating value*'? Semuanya calculating value kecuali plot dan write.csv yang mengubah environment dan menyimpannya.
